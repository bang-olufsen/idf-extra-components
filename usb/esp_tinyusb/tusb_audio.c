...
struct tinyusb_audio_handle {
    bool initialized;
    tusb_audio_cb_t on_pre_callback; // Note: MUST be super fast. Only call tud_audio_write.
    tusb_audio_cb_t on_post_callback;
    tusb_audio_cb_t on_get_sample_freq;
};
static struct tinyusb_audio_handle audio_obj = {};
...
{
    if ((cfg->on_post_callback == NULL) || (cfg->on_pre_callback == NULL)) {
        ESP_LOGE(TAG, "No audio callbacks provided");
        return ESP_FAIL;
    }

    audio_obj.on_pre_callback = cfg->on_pre_callback;
    audio_obj.on_post_callback = cfg->on_post_callback;
    audio_obj.on_get_sample_freq = cfg->on_get_sample_freq;
...
// This callback writes the buffer to the usb fifo. Must be super fast!
bool tud_audio_tx_done_pre_load_cb(uint8_t rhport, uint8_t itf, uint8_t ep_in, uint8_t cur_alt_setting)
{
    ...
    audio_obj.on_pre_callback();

    return true;
}


// This callback comes right after the callback above, giving the MCU time to prepare the new audio buffer to be sent next time
    ...
    if (entityID == 4) {
        ...
        case AUDIO_CS_CTRL_SAM_FREQ:
            ...
            case AUDIO_CS_REQ_CUR:
                ...
                if (audio_obj.on_get_sample_freq != NULL) {
                    audio_obj.on_get_sample_freq();
                }
